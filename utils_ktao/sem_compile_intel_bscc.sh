...
if [ ! -f "$sem_setup_dir/constants.h.in" ]
then
  rm -f constants.h.in
...
# ./configure FC=mpiifort MPIFC=mpiifort CC=mpiicc #MPI_INC="${TACC_IMPI_INC}" FCFLAGS="-xCORE-AVX2 -traceback" CFLAGS="-xCORE-AVX2 -traceback"
# ./configure FC=mpiifort MPIFC=mpiifort CC=mpiicc FCFLAGS="-O3" CFLAGS="-O3"
./configure FC=mpiifort MPIFC=mpiifort CC=mpiicc FCFLAGS="-xCORE-AVX2 -O3" CFLAGS="-xCORE-AVX2 -O3"

make clean
# make all  > >(tee make.out) 2> >(tee make.err >&2)
make xmeshfem3D xspecfem3D xcombine_vol_data_vtk > >(tee make.out) 2> >(tee make.err >&2)
