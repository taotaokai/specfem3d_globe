...
void FC_FUNC_(transfer_adj_to_device,
  ...
  for (irec = 0; irec < nrec; irec++) {
    if (mp->myrank == h_islice_selected_rec[irec]) {
      ...
      mp->h_source_adjoint[INDEX2(NDIM,0,irec_local)] = h_source_adjoint[INDEX2(NDIM,0,irec_local)];
      mp->h_source_adjoint[INDEX2(NDIM,1,irec_local)] = h_source_adjoint[INDEX2(NDIM,1,irec_local)];
      mp->h_source_adjoint[INDEX2(NDIM,2,irec_local)] = h_source_adjoint[INDEX2(NDIM,2,irec_local)];
