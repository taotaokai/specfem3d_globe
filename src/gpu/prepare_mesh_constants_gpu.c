  ...
  if (mp->nadj_rec_local > 0) {
    ...
    if (GPU_ASYNC_COPY) {
      ...
      if (run_cuda) {
        ...
        print_CUDA_error_if_any(cudaMallocHost((void**)&(mp->h_source_adjoint),(mp->nadj_rec_local)*NDIM*sizeof(realw)),6011);
      }
#endif
    } else {
      mp->h_source_adjoint = (realw *) malloc (mp->nadj_rec_local * NDIM * sizeof (realw));
      if (mp->h_source_adjoint == NULL) exit_on_error ("h_source_adjoint not allocated\n");
    }
