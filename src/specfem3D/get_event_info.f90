...
!--- arguments of the subroutine below
  ...
  if (USE_FORCE_POINT_SOURCE) then
    ...
    do isource = 1,NSOURCES
      ...
      ! read header with event information
      read(string(1:istart-1),*,iostat=ier) datasource
      if (ier /= 0) then
        write(IMAIN,*) 'Error reading datasource in source ',isource
        stop 'Error reading event info in CMTSOLUTION file'
      endif

      read(string(istart:iend),*,iostat=ier) yr,mo,da,ho,mi,sec,elat_pde,elon_pde,depth_pde,mb,ms
      if (ier /= 0) then
