...
! standard routine to setup model
  ...
  use constants, only: myrank, &
    USE_EXTERNAL_TOPO_410_660 ! ktao add
  ...
  ! ktao: read external 410-/660-km topo map
  if (USE_EXTERNAL_TOPO_410_660) then
    call topo%read_model_file("DATA/s362ani/external_topo410_660.txt")
  end if
...
!
  ...
  integer :: i,ihor,ifst,ilst,ios,idummy
  !<<<
  ...
  do while (ios == 0)
    ...
    if (ios == 0) then
      if (string(1:16) == 'REFERENCE MODEL:') then
        !>>>KTAO use adjustl to remove leading blanks
        ...
        !endif
        substr = adjustl(trim(string(17:)))
        if (len_trim(substr) == 0) then
          stop 'Error reading model 1'
        else
          refmodel = trim(substr)
        endif
        !<<<
        ...
        !endif
        substr = adjustl(trim(string(10:)))
        if (len_trim(substr) == 0) then
          stop 'Error reading model 4'
        else
          desckern(idummy) = trim(substr)
        endif
        !<<<
      else if (string(1:29) == 'HORIZONTAL PARAMETERIZATIONS:') then
        !>>>KTAO
        !substr=string(30:len_trim(string)) 
        substr = adjustl(trim(string(30:)))
        !<<<
        ...
        !  enddo
        !endif
        string = adjustl(trim(string(10:)))
        if (len_trim(string) == 0) then
          stop 'Error reading model 6'
        else if (string(1:20) == 'SPHERICAL HARMONICS,') then
          substr = trim(string(21:))
          read(substr,*) lmax
          ityphpar(idummy) = 1
          lmaxhor(idummy)=lmax
          ncoefhor(idummy)=(lmax+1)**2
        else if (string(1:18) == 'SPHERICAL SPLINES,') then
          ifst=len_trim(string)
          ilst=len_trim(string)
          do while(string(ifst:ifst) /= ',')
            ifst=ifst-1
          enddo
          read(string(ifst+1:ilst),*) ncoef
          hsplfile(idummy) = adjustl(trim(string(19:ifst-1)))
          ityphpar(idummy) = 2
          lmaxhor(idummy) = 0
          ncoefhor(idummy) = ncoef
          do i = 1,ncoef
