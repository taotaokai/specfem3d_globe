...
subroutine compute_coord_main_mesh(offset_x,offset_y,offset_z,xelm,yelm,zelm, &
  ...
  do ignod = 1,NGNOD
    ...
    if (ilayer == NUMBER_OF_MESH_LAYERS .and. INCLUDE_CENTRAL_CUBE) then
      ...
      !!KTAO: Scheme used to map node at (ix, iy, r) to the Cartesian coordinate
      !       ix and iy are the horizontal position measured in numbers of elements at surface.
      !
      !                      ETA
      !        Z           +
      !        |         +
      ...
      !        O (Earth's center)
      !
      !     N is the north pole, XI-N-ETA is the tangent plane at north pole.
      !     O-N-ETA is the meridian at zero-degree longitude  
      !     The non-dimensionalized Earth radius ON = 1
      !
      !     angle N-O-XI = ANGULAR_WIDTH_XI / 2, and angle N-O-ETA = ANGULAR_WIDTH_ETA / 2
      !     i.e., the horizontal center of the mesh region is at point N
      !
      !     angle N-O-A: theta_xi  = (ix / NEX_XI  - 1/2) * ANGULAR_WIDTH_XI  
      !     angle N-O-B: theta_eta = (iy / NEX_ETA - 1/2) * ANGULAR_WIDTH_ETA 
      !     ix and iy are the horizontal position measured in numbers of elements at surface.
      !     NEX_XI and NEX_ETA are the total number of elements along x and y direction at surface.
      !
      !     By setting vector ND = NA + NB, we define the normalized vector 
      !     n_OD = [tan(theta_xi), tan(theta_eta), 1] / norm2(OD) as the radial direction vector of node at (ix,iy) 
      ! 
      !     Thus, the node at (ix, iy, r) is mapped to r * [tan(theta_xi), tan(theta_eta), 1] / norm2(OD)
      ! 
      !     NOTE: transform to ECEF coordinates (XYZ) used in the code: X = -eta, Y = xi, Z = z        

      ratio_xi = ((iproc_xi + offset_x(ignod)/dble(NEX_PER_PROC_XI))/dble(NPROC_XI))  ! this is ix / NEX_XI
      x = 2.d0*ratio_xi-1

      ratio_eta = ((iproc_eta + offset_y(ignod)/dble(NEX_PER_PROC_ETA))/dble(NPROC_ETA))
      y = 2.d0*ratio_eta-1

      x = tan((ANGULAR_WIDTH_XI_RAD/2.d0) * x)        ! this is tan(theta_xi)
      ...
      gamma = ONE / sqrt(ONE + x*x + y*y)  ! this is 1 / norm2(OD)
      ...
      if (NCHUNKS /= 6) then
        ...
        !   enddo
        ! enddo
        vector_rotated = matmul(rotation_matrix, vector_ori)
        ...
        !    enddo
        ! enddo
        vector_rotated = vector_rotated * (r_top / r_bottom)
