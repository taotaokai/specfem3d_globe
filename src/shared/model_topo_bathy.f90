...
!
  ...
  !-- ktao: modified the code below to use gridline registration 
  !-- ktao: the topo is taken from etopo1_ice_g_gmt4.grd (smooth, resample)
  !-- ktao: topo(lon,kat): lon [-180,180], lat[-90,90]
  ...
  ! longitude within range [-180,180] degrees
  xlo = mod(xlon, 360.d0)
  if (xlo > 180.d0) xlo = xlo - 360.d0

  ! compute number of samples per degree
  ! ktao: this is actually the pixel size in degree
  samples_per_degree_topo = dble(RESOLUTION_TOPO_FILE) / 60.d0

  ! compute offset in data file and avoid edge effects
  iel1 = 1 + int((xlo+180.d0)/samples_per_degree_topo)
  if (iel1 < 1) iel1 = 1
  if (iel1 > NX_BATHY-1) iel1 = NX_BATHY-1

  iadd1 = 1 + int((xlat+90.d0)/samples_per_degree_topo)
  if (iadd1 < 1) iadd1 = 1
  if (iadd1 > NY_BATHY-1) iadd1 = NY_BATHY-1

  ! lower-left corner
  lon_corner = -180.d0 + (iel1-1)*samples_per_degree_topo
  lat_corner = -90.d0 + (iadd1-1)*samples_per_degree_topo
